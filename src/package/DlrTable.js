		...
		return h(
			"el-table-column",
			{
				props: {
					...
					filterMethod: column.filters
						? filtetag.bind(null, column)
						: null,
				},
...
const createTable = function(h) {
	...
	if (expand) {
		children.push(
			h("el-table-column", {
				...
				key: "expand",
				scopedSlots: {
					default: this.$scopedSlots.expand
						? (props) => this.$scopedSlots.expand(props)
						: null,
				},
			})
		);
	...
	if (showNum && list.length) {
		...
		children.push(
			h("el-table-column", {
				...
				key: "showNum",
				scopedSlots: {
					default: this.$scopedSlots.num
						? (props) =>
								this.$scopedSlots.mnum({
									...props,
									num: getNum(props),
								})
						: (props) => getNum(props),
	...
	if (eventFun.selectionChange)
		eventFun["selection-change"] = eventFun.selectionChange;
	return h(
		"el-table",
		{
			props: {
				...
				spanMethod:
					mergeRow.length || mergeColumn.length
						? arraySpanMethod
						: spanMethod,
				...$attrs,
			},
		...
		[].concat(
			children,
			this.$slots.default || [],
			createTableColumn.call(this, h, columns)
		)
	);
};
	...
	methods: {
		...
		summaryDefault(param) {
			...
			columns.forEach((column, index) => {
				...
				const obj = prop
					? this.columns.find((obj) => obj.prop === prop)
					: null;
				if (obj && sums[index]) {
					if (obj.isCurrency) {
		...
		arraySpanMethod({ row, column, rowIndex }) {
			...
			if (this.mergeColumn.length) {
				if (this.mergeColumn.indexOf(column.property) > -1) {
					...
					column.property &&
					this.getPropValue(row, column.property) === undefined
				) {
					colspan = 0;
				}
			}
		...
		rowDrop() {
			// const tbody = this.$el.querySelector(
