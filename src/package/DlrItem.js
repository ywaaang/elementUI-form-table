...
export default {
	...
	computed: {
		computedColumn() {
			return {
				...
				...((this.column.getColumn &&
					this.column.getColumn(this.row)) ||
					{}),
			};
		},
		componentType() {
			const { el } = this.computedColumn;
			const { isPreview, isReview } = this;
			if (!el || isPreview || isReview) return null;
			if (
				el === "dlrSelect" ||
				el === "DlrSelect" ||
				el === "select" ||
		...
		valueKey() {
			if (this.computedColumn.valueKey)
				return this.computedColumn.valueKey;
			return {
				label: "label",
				value: "value",
			};
		},
		modelHistoryComputed() {
			return this.getStrFunction(`this.row.${this.column.prop}_history`);
		},
		modelComputed: {
			get() {
				let val = null;
			...
			set(value) {
				...
				try {
					if (
						this.getStrFunction(
							`this.row.${this.computedColumn.prop}`
						) === undefined
					) {
						this.setRowKey(value);
					} else
						this.getStrFunction(
							`this.row.${this.computedColumn.prop} = value`
						);
				} catch (error) {
					this.setRowKey(value);
				}
	...
	methods: {
		...
		setRowKey(value) {
			if (this.computedColumn.prop && this.row) {
				const path = this.computedColumn.prop
					.replace(/\[(\w+)\]/g, ".$1")
					.replace(/^\./, "");
		...
		getParams() {
			const newObj = {};
			if (
				this.computedColumn.params &&
				typeof this.computedColumn.params === "object"
			) {
				for (const key in this.computedColumn.params) {
					let value;
					try {
						value = this.getStrFunction(
							`this.row.${this.computedColumn.params[key]}`
						);
					} catch (err) {}
					newObj[key] =
						value !== undefined
							? value
							: this.computedColumn.params[key];
				}
			}
			return newObj;
		},
		renderText(h) {
			...
			if (isReview) {
				const format =
					computedColumn.historyFormat || computedColumn.format;
				return [
					computedColumn.format
						? computedColumn.format(modelComputed, row)
						: modelComputed,
					modelHistoryComputed &&
						h(
							"span",
							...
							format(modelHistoryComputed, row)
								? format(
										modelHistoryComputed,
										row
								  )
		...
		if (componentType) {
			...
			const listeners = {
				...
				input: (val) => {
					...
					computedColumn.listeners &&
						computedColumn.listeners.input &&
						computedColumn.listeners.input(val);
				},
			};
			if (
				computedColumn.listeners &&
				computedColumn.listeners.currentObj
			) {
				listeners.currentObj = (data) =>
					computedColumn.listeners.currentObj(data, row, $index);
			}
			const arr = ["dlr-select", "el-checkbox-group", "el-radio-group"];
			if (arr.includes(componentType)) {
				...
				return h(
					componentType,
					{
						props: {
							...
							params:
								componentType === "dlr-select"
									? getParams(computedColumn)
									: null,
							value: modelComputed,
							customRender:
								computedColumn.slots &&
								computedColumn.slots.default
									? computedColumn.slots.default
									: null,
						},
						attrs:
				...
				if (
					...
					computedColumn.type === "currency"
				) {
					listeners.blur = (...args) => {
						this.isForce = false;
						computedColumn.listeners &&
							computedColumn.listeners.blur &&
							computedColumn.listeners.blur(...args);
					};
					listeners.focus = (...args) => {
						this.isForce = true;
						computedColumn.listeners &&
							computedColumn.listeners.focus &&
							computedColumn.listeners.focus(...args);
					};
				}
				...
				const attrs = attributes.reduce(
					(obj, key) => {
						if (this.computedColumn[key])
							obj[key] = this.computedColumn[key];
						return obj;
					},
					{ placeholder }
				);
